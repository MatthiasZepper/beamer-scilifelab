MAKEFLAGS  := -j 1
INS         = source/beamerthemescilifelab.ins
...
DOC_SRC     = doc/scilifelabtheme.dtx
DOC_PDF     = doc/scilifelabtheme.pdf

CTAN_CONTENT = README.md $(INS) $(PACKAGE_SRC) $(DOC_SRC) $(DOC_PDF) $(DEMO_SRC) $(DEMO_PDF)

DESTDIR     ?= $(shell kpsewhich -var-value=TEXMFHOME)
INSTALL_DIR  = $(DESTDIR)/tex/latex/scilifelab
DOC_DIR      = $(DESTDIR)/doc/latex/scilifelab
...
ctan: $(CTAN_CONTENT) ctan-version
	@tar --transform "s@\(.*\)@scilifelab/\1@" -cf scilifelab-$(shell date "+%Y-%m-%d").tar.gz $(CTAN_CONTENT)
